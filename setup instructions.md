...
- **Part 2: Detailed Steps** — full instructions for people who have never coded before, with explanations for every click.

Both parts cover the same ground, including downloading the FOLIO notebooks themselves. Use whichever part matches your experience level — you don't need to read both.
...
**Suggested order for running the notebooks:** Start with `folio_auth.ipynb` — this is where you enter your FOLIO login credentials. Then run `folio_auth_test.ipynb`, a short script to confirm you authenticated correctly, before moving on to any of the report notebooks (`fines and fees report.ipynb`, `instance records creation stats.ipynb`, `shelf list by location.ipynb`).
...
3. In VS Code, install the **Python** and **Jupyter** extensions (both published by Microsoft).
4. Download the notebooks:
   - Go to `https://github.com/ChristineMcClureEBSCO/Intro-to-FOLIO-APIS---Jupyter-Notebooks`
   - Either `git clone` the repo, or use **Code > Download ZIP** and extract it.
5. Open the resulting folder in VS Code: **File > Open Folder**.
...
A step-by-step guide for people who have never coded before. Follow these steps in order — don't skip ahead.
